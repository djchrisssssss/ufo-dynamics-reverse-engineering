...
\title{Reverse Engineering the Dynamics of Unidentified Aerial Phenomena:\\
A Physics-Based Assessment}

\author{Independent Research Initiative}
\email{analyst@example.org}
\orcid{0000-0000-0000-0000}
\affiliation{Independent Research Initiative --- Open Source Analysis}
...
known physics in principle; the constraints are engineering and materials science
maturity, not fundamental theory. The gap between theoretical possibility and
...
propulsion~\cite{Puthoff_DIRD,Davis_Wormhole_DIRD}.
...
effects)~\cite{Green_DIRD,Leonhardt_DIRD,Miley_DIRD,Shvets_DIRD}.
...
communications near UAP~\cite{Green_DIRD}.
...
an average acceleration of approximately $5{,}300\,g$~\cite{ODNI2021}.
...
curvature~\cite{Misner1973,Wald1984}. An object following
...
conventional propulsion systems~\cite{Birrell1982}.
...
electromagnetic waves around an object~\cite{Pendry2006}. Experimental
...
parameter $b$ is~\cite{Dyson1920,Kennefick2020}:
...
ship~\cite{Pfenning1997}. Van~Den Broeck~(1999) reduced the energy requirement to
...
regions persist when checked for all timelike observers~\cite{Santiago2022}.
...
advantage over conventional propulsion~\cite{Bobrick2021}. Fell and
Heisenberg~(2021) explored additional positive-energy geometries, further
contributing to the subluminal positive-energy landscape~\cite{Fell2021}.
...
magnitude~\cite{Pfenning1997}.

Quantum vacuum energy extraction was explored in DIRD~\#24~\cite{Davis_Vacuum_DIRD}
and DIRD~\#36~\cite{Davis_Tomography_DIRD}. While the quantum vacuum contains
...
(non-collapsing) wormhole~\cite{Morris1988}:
...
throat~\cite{Morris1988,Visser1995}.
...
(Einstein--Podolsky--Rosen pairs, ``EPR'')~\cite{Maldacena2013}. If correct, this
...
satisfy the Lawson criterion~\cite{Misner1973}:
...
exceedingly difficult with current technology~\cite{Culbreth_DIRD}.
...
\begin{itemize}
  ...
  \item \textbf{Inertial electrostatic confinement~(IEC):} Explored in
        DIRD~\#9~\cite{Miley_DIRD}---uses electrostatic fields to accelerate and
        confine ions.
\end{itemize}
...
conventional energy source. DIRD~\#24~\cite{Davis_Vacuum_DIRD} and
DIRD~\#36~\cite{Davis_Tomography_DIRD} explored this question without identifying
...
\SIrange{30}{\mu m}{3}{mm}) occupies the ``THz gap'' between microwave electronics
...
DIRD~\#26~\cite{Green_DIRD} documented patterns of biological effects reported by
...
waves around an object~\cite{Pendry2006}. The required material parameters are
...
limited~\cite{Shelby2001,Leonhardt_DIRD,Shvets_DIRD}.
...
Even the most optimistic estimates~\cite{Bobrick2021} for a positive-energy
...
observable universe~\cite{Pfenning1997}. Van~Den Broeck's
...
\begin{description}
  ...
  \item[UAP] Unidentified Aerial Phenomena.
...
Level & Definition \\
\midrule
High & Well-established physics with extensive experimental verification,
  multiple independent confirmations, broad scientific consensus. \\
Moderate & Theoretically sound frameworks with partial experimental support, or
  established physics in novel contexts. \\
Low & Unverified theories, limited data, or speculative extrapolation from
  established physics. \\
\bottomrule
\end{tabular}
\end{table}
...
Portions of this manuscript were prepared with AI-assisted tools for literature
synthesis, equation typesetting, and structural organization. All scientific
judgments, assessments, and conclusions are those of the author.

This work is released under the Creative Commons Attribution~4.0 International
License (CC~BY~4.0).

Document Identifier: IRD-UAP-2026-001. Version~1.0, March~2026.
\end{acknowledgments}
